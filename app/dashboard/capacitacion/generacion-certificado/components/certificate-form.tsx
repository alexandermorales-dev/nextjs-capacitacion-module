...
export default function CertificateForm({
  ...
  const [newParticipant, setNewParticipant] = useState({ name: '', id_number: '', score: 0 })
  ...
  const removeParticipant = (id: string) => {
    onParticipantsChange(certificateData.participants.filter(p => p.id !== id))
  }
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6">
      ...
      {selectedOSI && (
        <div className="mb-6 p-4 bg-gray-50 rounded-lg">
          <h3 className="text-sm font-medium text-gray-700 mb-2">Información de la OSI</h3>
      ...
      {/* Certificate Title */}
      <div className="mb-4">
        <label htmlFor="certificate_title" className="block text-sm font-medium text-gray-700 mb-2">
      ...
      <div className="mb-4">
        <label htmlFor="certificate_subtitle" className="block text-sm font-medium text-gray-700 mb-2">
          Subtítulo (Opcional)
        </label>
        <input
          type="text"
      ...
      {/* Passing Grade */}
      <div className="mb-4">
        <label htmlFor="passing_grade" className="block text-sm font-medium text-gray-700 mb-2">
        ...
        <input
          ...
          onChange={(e) => onDataChange('passing_grade', parseInt(e.target.value) || 14)}
          min="0"
          max="20"
          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        />
        <p className="text-xs text-gray-500 mt-1">
          Los participantes con calificación mayor o igual a este valor serán marcados como "Aprobado"
        </p>
      </div>
        ...
        <div className="relative">
          <div className="relative">
            <input
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              placeholder="Escribe para buscar ciudad o estado..."
            />
            {locationInput && (
              <button
                type="button"
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-2 mb-3">
          ...
          <input
            ...
            onChange={(e) => setNewParticipant(prev => ({ ...prev, id_number: e.target.value }))}
            onKeyPress={handleKeyPress}
          ...
          <input
            ...
            onChange={(e) => setNewParticipant(prev => ({ ...prev, score: parseInt(e.target.value) || 0 }))}
            onKeyPress={handleKeyPress}
              ...
              >
                ...
                >
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 5.14M12 16l-7 7m0 0l7 7-7" />
                  </svg>
                </button>
              </div>
            ))}
      ...
      <button
        type="button"
        onClick={onGenerate}
        disabled={!certificateData.certificate_title || !certificateData.osi_id || 
                  !certificateData.course_topic_id || certificateData.participants.length === 0 ||
                  !certificateData.location || !certificateData.date}
      ...
      {/* Validation Message */}
      {(!certificateData.certificate_title || !certificateData.osi_id || 
        !certificateData.course_topic_id || certificateData.participants.length === 0 ||
        !certificateData.location || !certificateData.date) && (

...
export default function OSIDashboardMetrics({
  ...
  const totalOSIs = totalCount !== undefined ? totalCount : osis.length;
  const totalHours =
    metrics?.total_hours ??
    osis.reduce((sum, osi) => sum + (osi.horas_academicas_ejecucion || 0), 0);
  const totalSesiones =
    metrics?.total_sesiones ??
    osis.reduce((sum, osi) => sum + (osi.sesiones_ejecucion || 0), 0);
  const uniqueCompanies =
    metrics?.unique_companies ??
    new Set(osis.map((osi) => osi.id_empresa)).size;

  // Get the most common status
  const statusCounts = osis.reduce(
    (acc, osi) => {
      acc[osi.status_name || "Desconocido"] =
        (acc[osi.status_name || "Desconocido"] || 0) + 1;
      return acc;
    },
  ...
  const mostCommonStatusColor =
    statuses.find((s) => s.nombre_estado === mostCommonStatus?.[0])
      ?.color_hex || "#6B7280";
